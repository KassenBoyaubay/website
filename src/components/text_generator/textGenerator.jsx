...
const TextGenerator = () => {
  ...
  const changed = (e) => {
    if (e.currentTarget.value <= Alltext.length && e.currentTarget.value > 0)
      setNumber(e.currentTarget.value);
  };

  const generate = (e) => {
    e.preventDefault();
    setText(Alltext.slice(0, number));
  };

  const generateRandom = (e) => {
    let random = Math.floor(Math.random() * Alltext.length);
    setNumber(random);
  };

  return (
    <div className="mySass TextGenerator">
      <section class="section-center">
        ...
        >
          ...
          <button class="btn" onClick={(e) => generateRandom(e)}>
